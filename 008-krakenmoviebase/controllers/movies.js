...
module.exports = function (router) {
    ...
    router.post('/search', function (req, res) {
        ...
        Movie.search(req.body.searchText, {title: 1, plot: 1, cover: 1}, {
            conditions: { title: { $exists: true}, plot: {$exists: true}, cover: {$exists: true}}, sort: {title: 1}, limit: 10 }, function (err, movies) {
            if (err) {
                res.send(err);
            } else {
                var model = {
    ...
    router.post('/add', function (req, res) {
        ...
        if (title === '' || release_date === '') {
            req.flash('error', "please fill out require(*) fields");
